#!/bin/sh
# =============================================================================
# Xcode Cloud — Flutter setup for the macOS workflow.
#
# Same role as ios/ci_scripts/ci_post_clone.sh. Xcode Cloud looks for
# ci_scripts next to the Xcode product it builds: a workflow on
# macos/Runner.xcworkspace uses THIS file, not the iOS one.
#
# Must stay executable IN git:
#   git update-index --chmod=+x macos/ci_scripts/ci_post_clone.sh
# =============================================================================
set -eu
...
echo "=== Xcode Cloud: Flutter $FLUTTER_VERSION ($FLUTTER_CHANNEL, macOS) ==="
...
echo "--- flutter build macos --config-only (build number $BUILD_NUMBER)"
# shellcheck disable=SC2086
flutter build macos --config-only --release --build-number="$BUILD_NUMBER" $DART_DEFINES

echo "=== Ready (SwiftPM). Xcode Cloud can archive macOS. ==="
